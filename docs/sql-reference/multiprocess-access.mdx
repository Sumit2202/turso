...
- **WAL journal mode only**: MVCC keeps its transaction state and `.db-log` writer/checkpoint coordination inside one process. Opening an existing MVCC database or running `PRAGMA journal_mode=mvcc` with `multiprocess_wal` enabled returns `InvalidArgument`.
...
- Attached databases (via [ATTACH DATABASE](/docs/sql-reference/statements/attach-database)) inherit the multi-process setting of the main connection.
- The feature is not supported under MVCC (`BEGIN CONCURRENT`). The engine rejects this combination before opening or creating the MVCC logical log. Use either MVCC within a single process or multi-process WAL with the default isolation model, not both.
...
- [Experimental Features](/docs/sql-reference/experimental-features) — how to enable experimental features across all surfaces
- [Transactions](/docs/sql-reference/statements/transactions) — WAL concurrency model and `BEGIN CONCURRENT`
- [Compatibility](/docs/sql-reference/compatibility) — SQLite compatibility matrix
